...
class Cointopay_IbanCallbackModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
        ...
        try {
            ...
            if (isset($response_ctp)) {
                if ($response_ctp->data['Security'] != $ConfirmCode) {
                    ...
                    if (_PS_VERSION_ >= '1.7') {
                        $this->setTemplate('module:cointopay_iban/views/templates/front/cointopay_payment_cancel.tpl');
                    } else {
                ...
                } elseif (null != Tools::getValue('COINTOPAY_IBAN_MERCHANT_ID') && $response_ctp->data['MerchantID'] != Tools::getValue('COINTOPAY_IBAN_MERCHANT_ID')) {
                    ...
                    if ($ctp_order_status == 'paid' && $ctp_order_status_notenough == 0) {
                        ...
                        $order_status = 'COINTOPAY_IBAN_WAITING';
                    ...
                    if ($order_status !== false && $order_status == 'PS_OS_PAYMENT') {
                        ...
                        if (_PS_VERSION_ >= '1.7') {
                            $this->setTemplate('module:cointopay_iban/views/templates/front/ctp_payment_callback.tpl');
                        } else {
                            $this->setTemplate('ctp_payment_callback.tpl');
                        }
                    } elseif ($order_status == 'COINTOPAY_IBAN_NOTENOUGH') {
                        $history = new OrderHistory();
                        $history->id_order = $order->id;
                        $history->changeIdOrderState((int) Configuration::get($order_status), $order->id);
                        ...
                        if (_PS_VERSION_ >= '1.7') {
                            $this->setTemplate('module:cointopay_iban/views/templates/front/ctp_payment_paid_notenough.tpl');
                        } else {
                            $this->setTemplate('ctp_payment_paid_notenough.tpl');
                        }
                    } elseif ($order_status == 'COINTOPAY_IBAN_FAILED') {
                        $history = new OrderHistory();
                        $history->id_order = $order->id;
                        $history->changeIdOrderState((int) Configuration::get($order_status), $order->id);
                        $history->addWithemail(true, ['order_name' => explode('----', Tools::getValue('CustomerReferenceNr'))[0]]);
                        ...
                        if (_PS_VERSION_ >= '1.7') {
                            $this->setTemplate('module:cointopay_iban/views/templates/front/ctp_payment_failed.tpl');
                        } else {
                            $this->setTemplate('ctp_payment_failed.tpl');
                        }
                    } elseif ($order_status == 'COINTOPAY_IBAN_EXPIRED') {
                        $history = new OrderHistory();
                        $history->id_order = $order->id;
                        $history->changeIdOrderState((int) Configuration::get($order_status), $order->id);
                        $history->addWithemail(true, ['order_name' => explode('----', Tools::getValue('CustomerReferenceNr'))[0]]);
                        ...
                        if (_PS_VERSION_ >= '1.7') {
                            $this->setTemplate('module:cointopay_iban/views/templates/front/ctp_payment_expired.tpl');
                        } else {
                        ...
                        Tools::redirect($this->context->link->getModuleLink('cointopay_iban', 'cancel'));
                    } else {
